...
extern void executionStart(char*);
...
ExceptionHandler(ExceptionType which)
{
    ...
    int size_allocate;    //used for reading register in SYScall_ShmAllocate
    unsigned start_shared_memory;   //used in SYScall_ShmAllocate
    int value_return;
	...
	DEBUG('a', "Shutdown, initiated by user program.\n");
       ...
       value_return = FALSE;
       while(value_return != TRUE){
            value_return = machine->ReadMem(vaddr, 1, &memval);
       }
       ...
       size_allocate = machine->ReadRegister(4);
       int pages_created;

       // this will create a new page table with shared pages
       start_shared_memory = (unsigned)currentThread->space->makeSharedPageTable(size_allocate, &pages_created);

       // Increase the number of page faults by number of new pages created
       stats->numPageFaults += pages_created;
       ...
       machine->WriteRegister(2, start_shared_memory);
    }
    else if (which == PageFaultException) {
        //thread should be BLOCKED now and after that it goes to sleep
        //for 100 ticks (page fault latency)
