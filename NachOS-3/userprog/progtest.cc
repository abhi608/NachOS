...
//following function is used to execute a process

void executionStart(char *filename)
{
    OpenFile *executable = fileSystem->Open(filename);
    ProcessAddrSpace *space;
    ...
    delete currentThread->backup_memory;
    currentThread->space->setPagesFree(TRUE);
    space = new ProcessAddrSpace(executable);    
    currentThread->space = space;
    strcpy(space->filename, filename);
    space = new ProcessAddrSpace(executable);    
    ...
    // delete currentThread->page_cache;  //delete the page cache of current thread before 
    //                                     //allocating it a new address space
    // if(page_algo != NORMAL) {
    //     currentThread->space->setPagesFree(TRUE);
    //     delete currentThread->backup_memory;  //time to delete the old backup memory
    // }

    // //Now, create  anew address space for the current thread and pass it
